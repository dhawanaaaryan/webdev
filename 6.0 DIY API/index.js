...
app.get("/filter",(req,res)=>{
  const i=req.query.type;
  const result=jokes.find((joke)=>joke.jokeType===i);
  res.json(result);
})
...
app.put("/jokes/:id",(req,res)=>{
  ...
  const ind=jokes.findIndex((joke)=>joke.id===req.params.id);
  jokes[ind]=j;
  res.json(j);
...
app.patch("/jokes/:id", (req, res) => {
  ...
  // Check if joke with given ID exists
  if (ind === -1) {
  ...
  if (req.body.text !== undefined) {
    jokes[ind].jokeText = req.body.text;
  }

  if (req.body.type !== undefined) {
    jokes[ind].jokeType = req.body.type;
  }
